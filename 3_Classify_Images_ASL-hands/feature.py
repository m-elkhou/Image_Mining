...
class Feature:
    ...
    def getFeatures(self, imagePath):
    ...
    def normalization_train(self, data_train, target, len_ft):
        X_train_transformed = self.pca.fit_transform(data_train)
        X_train_transformed = self.scaler.fit_transform(X_train_transformed)
        # X_train = fs.fit_transform(X_train_transformed, target)

        dic = { val : nb for nb, val in enumerate(set(target))}
        target2nb = [dic[l] for l in target]
        X_train_transformed = np.array(X_train_transformed )
        target2nb = np.array(target2nb)
        X_train = self.relief.fit_transform(X_train_transformed, target2nb)
        return X_train

    def normalization_test(self, data_test, len_ft):
        data_test = np.array(data_test)
        X_test_transformed = self.pca.transform(data_test)
        X_test_transformed = self.scaler.transform(X_test_transformed)
        X_test_transformed = np.array(X_test_transformed)
        X_test = self.relief.transform(X_test_transformed)
        return X_test

    def list_2_df(self, data_lst, target_lst):
        clm = [i for i in range(len(lst))]
